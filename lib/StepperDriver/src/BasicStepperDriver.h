...
public:
    ...
    static inline void delayMicros(unsigned long delay_us, unsigned long start_us = 0){
        if (delay_us){
            if (!start_us){
                start_us = (unsigned long)esp_timer_get_time();  // Replace micros()
            }
            if (delay_us > MIN_YIELD_MICROS){
                taskYIELD();  // Replace yield()
            }
            // See https://www.gammon.com.au/millis
            while ((unsigned long)esp_timer_get_time() - start_us < delay_us);  // Replace micros()
        }
    }
